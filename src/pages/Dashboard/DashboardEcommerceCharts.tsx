...
const StoreVisitsCharts = ({ chartId, details }: any) => {
  const chartColors = useChartColors(chartId);

  // Remove % symbol from dailyIncome values and convert to numbers
  const series =
    Object.values(details?.dailyIncome ?? {}).map((value: any) => {
  ...
  var options: any = {
    labels: Object.keys(details?.dailyIncome ?? []) ?? [],
    chart: {
      height: 333,
      type: "donut",
    },
